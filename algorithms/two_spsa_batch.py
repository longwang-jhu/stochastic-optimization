...
class TwoSPSABatch(OptiAlgo):
    def __init__(self, a=0, A=0, alpha=0.602, c=0, gamma=0.101, w=0,
        ...
        super(TwoSPSABatch, self).__init__(
            a, A, alpha, c, gamma,
    ...
    def get_grad_Hhat_est(self, iter_idx=0, rep_idx=0, theta_k=None):
        ...
        for dir_idx in range(self.dir_num):
            ...
            losst_plus = self.loss_noisy(sample_idx, theta_k + c_k * delta_k + c_k * deltat_k)
            losst_minus = self.loss_noisy(sample_idx, theta_k - c_k * delta_k + c_k * deltat_k)
